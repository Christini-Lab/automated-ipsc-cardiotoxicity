...
from seaborn import pointplot, swarmplot, histplot, distplot
import pickle
...
def plot_figure():
    ...
    axs = axs.flatten()
    [axs[i].set_ylabel('Count') for i in range(1,4)]

    plot_aps(axs[0], by_lab=True)

    plot_ap_features(axs[1], 'APD90', by_lab=True)
    plot_ap_features(axs[2], 'MDP', by_lab=True)
    plot_ap_features(axs[3], 'dVdt', by_lab=True)
    ...
    plt.savefig(f'./figure-pdfs/f-wu_vs_icell_ap.pdf')
    plt.show()


def plot_aps(ax, by_lab=True):
    ...
    if by_lab:
        wu_files = [f for f in all_files if '230403' not in f]
        icell_files = [f for f in all_files if '230403' in f]

        for f in wu_files:
            ap_dat = pd.read_csv(f'./data/cells/{f}/ap_df.csv')
            ax.plot(times[st_idxs:num_idxs], ap_dat.iloc[:, 4].values[st_idxs:num_idxs], color='#1f77b4', alpha=.3)
        for f in icell_files:
            ap_dat = pd.read_csv(f'./data/cells/{f}/ap_df.csv')
            ax.plot(times[st_idxs:num_idxs], ap_dat.iloc[:, 4].values[st_idxs:num_idxs], color='#ff7f0e', alpha=.3)
    else:
        for f in all_files:
            ap_dat = pd.read_csv(f'./data/cells/{f}/ap_df.csv')
...
def plot_ap_features(ax, feature, by_lab=False):
    all_files = get_valid_cells('dmso') + get_valid_cells('flecainide') + get_valid_cells('quinine')
    ...
    for f in all_files:
        ...
        if '230403' in f:
            icell_vals.append(feature_val)
        else:
            wu_vals.append(feature_val)
        #all_vals.append(feature_val)


    all_vals = icell_vals + wu_vals
    print(f'The mean and SD of {feature} is {np.mean(all_vals)} +- {np.std(all_vals)}')

    if by_lab == True:
        labels = ['Greenstone', 'iCell']
        #colors = ['r', 'g']

        for i, vals in enumerate([wu_vals, icell_vals]):
            distplot(vals, ax=ax, label=labels[i], bins=np.linspace(np.min(all_vals), np.max(all_vals), 10), kde=False)
            #distplot(vals, ax=ax, label=labels[i], kde=False)
        ax.legend()
    else:
