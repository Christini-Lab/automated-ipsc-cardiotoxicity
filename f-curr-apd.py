...
def plot_figure_apd():
    ...
    plot_curr(axs[2], 'flecainide', window, channel, is_change=is_change, min_or_max=min_or_max)
    plot_curr(axs[3], 'quinine', window, channel, is_change=is_change, min_or_max=min_or_max)
...
def plot_curr(ax, drug, window, channel, min_or_max, is_change=False):
    ...
    for f in all_files:
        ...
        if min_or_max == 'min':
            avg_vals = vc_slice.min().values / vc_meta['cm'].values
        else:
            avg_vals = vc_slice.max().values / vc_meta['cm'].values


        avg_vals = [np.mean(avg_vals[i*2:i*2+2]) for i in range(0, 5)]

        if channel == 'I_Na':
            if np.min(avg_vals) > -20:
        ...
        for i, compound in enumerate(['Baseline', '3xEFPC', '10xEFPC',
                                                  '20xEFPC', 'Washout']):
            vc_dict[compound].append(avg_vals[i])
